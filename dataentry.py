...
db_session.commit()
test_rating = RatingTag("He's great at tutoring Computer Science!", 4, db_session.query(User.id).where(User.name == "galen").first()[0], db_session.query(Tutor.id).where(Tutor.user_id == db_session.query(User.id).where(User.name == "colin").first()[0]).first()[0])
test_subject = SubjectTag(db_session.query(Subject.id).where(Subject.name == "Latin").first()[0], db_session.query(Tutor.id).where(Tutor.user_id == db_session.query(User.id).where(User.name == "galen").first()[0]).first()[0])
db_session.add_all([test_rating, test_subject])
db_session.commit()